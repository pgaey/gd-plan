...
| 항목 | 요점 | 상태 |
|---|---|:---:|
| `src/cli.ts` 설치기 축소 | get.sh 대체 후 deprecate 표기(@deprecated + 실행 배너) + README curl\|bash 설치 안내 | ✅ `9564d72` |
| release 절차 문서화 | 수동 semver bump 기준 + 체크리스트 → `docs/RELEASE.md` | ✅ `af5cc2c` |
...
- [x] 모든 SPEC 이 merge (base 모드: spec-02-01·02·03 → `phase-02-distribution`)
- [~] 통합 테스트 3 시나리오 — 1·2 PASS / 3(미용실 e2e)은 인터랙티브 스킬 실행 항목으로 사용자 별도 세션 (phase-01 C1 연속)
- [x] 성공 기준 1~5 정량 측정 (아래 검증 결과)
- [ ] 사용자 최종 승인 (phase PR 머지)

## 📊 검증 결과 (phase-ship 2026-06-10)

### 성공 기준 측정 (5/5)
| # | 기준 | 결과 | 증거 |
|---|---|:---:|---|
| 1 | curl\|bash 1회 footprint 전체 설치 + node 미설치 | ✅ | `get.sh --src` → 스킬 9·컬렉션 66·VERSION 0.1.0, `shasum -c` OK / test-get 시나리오 4(node 비의존 PATH) PASS |
| 2 | `gd status` 설치 vs 원격 최신 판별 | ✅ | **실제 raw 네트워크** 비교 → "✓ 최신 (0.1.0)" / test-gd 시나리오 3 업그레이드 가능 판정 |
| 3 | `gd upgrade` 수정 파일 손실 0 | ✅ | test-gd 시나리오 4 — 원본 보존 + `.new` + docs diff 0 + VERSION 재기록 |
| 4 | 컬렉션 전체 동봉 + 픽 네트워크 0 | ✅ | footprint 66 동봉 + design 스킬 네트워크 지시 부재 테스트 |
| 5 | vitest 회귀 + bash 테스트 PASS | ✅ | vitest 67/67 + 셸 9건 + build success |

### 통합 테스트 (2/3)
- 시나리오 1 (fresh, node 없는 환경): ✅ test-get 4건
- 시나리오 2 (업그레이드 + 충돌 보존): ✅ test-gd 5건
- 시나리오 3 (미용실 e2e): ⏳ 사용자 별도 세션 (설치본 스킬 인터랙티브 실행 = phase-01 C1)

### 비고
- **네트워크 tarball 전체 경로**(`get.sh` → `main.tar.gz`)는 본 phase PR 이 main 에 머지된 *후* 검증 가능 — install.sh 가 main 에 올라가야 tarball 에 포함됨(첫 release 닭-달걀, 코드 버그 아님). raw fetch·`--src`·`gd status` 네트워크는 실증 완료.
- phase-FF 2건(README·release 문서) 완료: `9564d72`, `af5cc2c`.

...
### 1-1. 브랜치 생성
- [ ] `git checkout -b spec-x-readme-overhaul` (main 에서, 작업트리의 queue.md 변경 carry-over)
- [ ] Commit: 없음 (브랜치 생성만)

### 1-2. phase-02 done 마커 커밋 (chore)
- [ ] `backlog/queue.md` 의 phase-02 done 마커(이미 존재) 스테이징
- [ ] Commit: `chore(spec-x-readme-overhaul): phase-02 done queue 마커 반영`

### 1-3. spec 산출물 커밋 (docs)
- [ ] spec.md / plan.md / task.md / critique.md 스테이징
- [ ] Commit: `docs(spec-x-readme-overhaul): spec/plan/task/critique 작성`
...
### 2-1. 교차검증 자료 수집
- [ ] 슬래시 9 (**`plans/gd-plan-*.md`**), gd CLI(`.gd/bin/gd`), get.sh 옵션, 카운트(스킬9/ADR16/컬렉션66/버전 `package.json` 0.1.0) 실제값 확인

### 2-2. README 재구성+증강
- [ ] `README.md` 개편 — 정확본문 재사용 + 누락 온보딩 절 신규(이게무엇/왜·언제쓰나·시작하기 mermaid) + stale 2건 교정 + ToC. spec.md FR1 절 구성
- [ ] 수동 검증 시나리오 1~4 (명령 1:1·죽은링크 0·카운트 일치·적용조건 존재) 확인
- [ ] Commit: `docs(spec-x-readme-overhaul): README 재구성+증강 — 소비자 온보딩`

...
> **비가역 결정 표기 (필수)**: 보안·법·규제·데이터모델·제품행동 등 *되돌리기 어려운(비가역)* 결정은 단정 금지 — 결정 라인에 `(사용자 확인 권장)` 마커를 붙여 사람이 잡을 수 있게 드러낸다(가역 UI 결정은 단정 가능). 위임("쭉 진행해")이 단정의 면허는 아니다.
...
- **자동 진행 (confirm-then-advance, loop)**: sitemap 로스터에서 아직 `done` 이 아닌 페이지가 있는지 본다.
  - **남은 todo 페이지가 있으면** → 다음 페이지 slug 를 가리키며 "다음 페이지 `<slug>` 이어서 할까요?"라고 묻고, **긍정** 시 `.claude/commands/gd-plan-page.md` 를 그 slug 로 다시 읽어 이어 실행(루프).
  - **모든 페이지가 done 이면** → "모든 페이지 완료 — 사용자 여정(`/gd-plan-flows`)으로 갈까요?"라고 묻고, **긍정** 시 `.claude/commands/gd-plan-flows.md` 를 읽어 이어 실행(루프 종료 → flows).
  - **부정/모호** → 정지. 슬래시 커맨드만 남긴다.

<!-- gd:advance next=loop -->

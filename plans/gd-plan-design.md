...
4. 후보 3개의 원본을 컬렉션 디렉토리에서 **그때 풀로드**해 사용자에게 비교 제시. 파일명은 인덱스 `file` 필드 **그대로** 사용 (이미 확장자 포함 — `.md` 를 덧붙이지 말 것):
   ...
   ```
4.5. **무드 스와치 프리뷰 결합 (ADR-019)**: 텍스트 목록과 함께, 후보 3개의 **사전 빌드된 스와치 fragment** 를 결합해 `docs/_design-preview.html` 를 만든다 — 사용자가 브라우저로 열어 톤을 *눈으로* 비교한다.
   - 컬렉션 디렉토리의 `_swatches/<file>.html`(예: `_swatches/cal.html` — `.md` 를 `.html` 로) 3개를 **그대로 읽어** grid 컨테이너로 감싸 결합한다. fragment 는 빌드타임 생성물이라 **새로 만들지 말고 읽어 붙이기만** 한다(node 불필요·결정적).
   - 상단에 **정직 배너**: "색·radius·그림자는 충실. **타이포는 폰트명 라벨**일 뿐 실제 폰트 렌더가 아님(독점/오픈소스 차등 렌더 편향 방지)."
   - `_swatches/` 가 없으면(구버전 설치) 경고 후 텍스트 목록만으로 진행 — dev: `pnpm build-swatches` / 소비자: `gd upgrade` 안내.
   - 출력: "프리뷰를 브라우저에서 열어 비교하세요(더블클릭): `docs/_design-preview.html`" (auto-open 안 함 — node-free·크로스플랫폼).
   - `_design-preview.html` 은 `_`-prefix 산출물(start/review auto-load 무시). 소비자 `.gitignore` 권고 대상(임시 산출물, 커밋 불필요). 재실행 시 덮어쓰기(멱등).

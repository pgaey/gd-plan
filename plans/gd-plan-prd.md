---
name: gd-plan-prd
description: PRD 인터뷰. 구조화 15문항으로 docs/prd.md(상류 SSOT)를 채우고 .gd/memory/project.md 요약을 동기화한다. capability에 [CAP-NN] ID + 주체 역할 부여. idempotent.
---

# gd-plan-prd — PRD 인터뷰

> 본 스킬은 *인터뷰어*입니다. 무작위 대화가 아니라 **구조화 15문항**으로 prd.md 의 모든 필드를 빠짐없이 채웁니다.
> **핵심 규칙 — 질문 ↔ prd 필드 완전 커버**: prd 템플릿의 모든 필드는 최소 1개 질문으로 커버된다. 템플릿에 필드를 추가하면 여기 질문도 추가.

---

## §1 자동 로딩 컨텍스트

| 파일 | 역할 |
|---|---|
| `docs/prd.md` (있으면) | 기존 초안 — 이어서 채움 (idempotent) |
| `.gd/memory/project.md` (있으면) | 이전 세션 프로젝트 컨텍스트 |
| `templates/prd.md` (패키지) | 필드 구조 = 질문의 source of truth |

## §2 인터뷰 진행 규칙

- **한 번에 1~2문항.** 답을 prd.md 초안에 즉시 반영하며 진행 (멱등 — 재실행 시 기존 답 보존, 빈 필드만 질문).
- 답이 추상적이면 **후보를 복수 선택형으로 제시** (manyfast 식 구조화). 예: 톤 → "친근 / 전문 / 미니멀 / 대담 / 신뢰".
- capability(Q10)마다 **주체 역할을 Q8 roles 에서 고르게** 한다 — `role -> capability` 연결을 인터뷰가 강제.
- (선택) 표시 질문은 사용자가 "없음" 하면 건너뛴다.

## §3 질문 세트 (15문항)

| # | 질문 | 형식 | → prd 필드 |
|---|---|---|---|
| Q1 | 한 줄로, 무엇을 만드나? | 서술 | 한 줄 요약 |
| Q2 | (선택) 왜 지금 이걸? | 서술 | 배경/맥락 |
| Q3 | 누구를 위해? (타깃 사용자) | 복수 | 페르소나 (1-3) |
| Q4 | 그들이 겪는 핵심 문제? | 복수 | (배경/핵심가치 근거) |
| Q5 | 핵심 목표·가치? | 단일 | 핵심 가치 |
| Q6 | (선택) 대안과 뭐가 다른가? | 서술 | 차별점 |
| Q7 | 톤·보이스는? | 복수 | 톤 키워드 (+ design 픽 힌트) |
| Q8 | 역할이 나뉘나? 각 역할이 뭘 하나? | 복수 | roles |
| Q9 | 접근 모델 + 디자인 함의? | 단일 | access model (RBAC/ABAC/ReBAC) |
| Q10 | 할 수 있어야 하는 것? (주체 역할 지정) | 복수 | capabilities `[CAP-NN]` (+주체) |
| Q11 | 성공을 어떻게 측정? | 복수 | 성공 기준 (정량 우선) |
| Q12 | 어떤 환경/기기? | 복수 | 기기/플랫폼 |
| Q13 | (선택) 비기능 요구? | 복수 | 비기능 (성능/접근성/다국어) |
| Q14 | 안 하는 것? | 복수 | Out of Scope |
| Q15 | (선택) 주요 리스크? | 복수 | 리스크 |

> 커버리지 체크 — prd 필드 전부 매핑: 한줄/배경/페르소나/핵심가치/차별점/톤/roles/access/capabilities(+주체)/기기/비기능/성공기준/리스크/Out of Scope (빠진 필드 0).

## §4 capability ID 규칙

- 각 capability: `[CAP-NN]` (2자리, 01부터) + **동사로 시작** + `(주체: 역할)`.
- 주체 역할은 반드시 Q8 의 roles enum 중 하나 (없는 역할이면 다시 확인).
- 예: `[CAP-01]` 시술 예약하기 (주체: User)

## §5 산출 + 메모리 동기화

1. `docs/prd.md` 작성/갱신 (템플릿 구조 따름).
2. `.gd/memory/project.md` 의 프로젝트 요약을 prd 핵심(한줄/페르소나/capabilities/톤)으로 **자동 갱신** (멱등). prd 가 SSOT, project.md 는 파생 요약.
3. **결정 기록 (자동 + 수동)**: 인터뷰 fork(Out-of-scope·톤·access 등)에서 하나를 고르면 `docs/decisions.md` 에 typed 1행 append (연결=`[CAP-..]`). 형식·ID·supersede 는 `docs/decisions.md` 헤더 / `ADR-011` 정본 참조. fork 밖 중요한 일회성 결정은 "이건 남길까요?" 제안 후 기록(수동 보강).

## §6 (선택) prd-extractor 서브에이전트

자유 발언이 길면 Sonnet 서브에이전트로 capability 후보를 추출해 Q10 을 돕는다.
> **non-normative**: 서브에이전트 없이도 동작해야 한다 (권장 힌트, DoD 강제 아님).

## §7 종료

- prd.md 의 필수 필드(한줄/페르소나/핵심가치/roles/capabilities/톤/성공기준/Out of Scope)가 모두 채워졌는지 확인.
- 미완 필드가 있으면 `<!-- TODO: 필드명 -->` 마커를 남기고 보고 (재개 가능).
- 출력: `docs/prd.md 작성 완료. 다음 단계: /gd-plan-design. 전체 진행률: 1/5`

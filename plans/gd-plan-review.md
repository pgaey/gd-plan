...
- **게이트 (자동 진행 금지)**: review 는 검증 게이트다. 다음 단계를 **자동으로 이어 실행하지 않는다**.
  - **BLOCK 있으면** → 정지하고, 되돌릴 지점(보통 `/gd-plan-page <slug>` 또는 해당 단계)을 안내한다. 자동 진행·자동 우회 금지(`--force-continue` 는 사용자가 명시할 때만).
  - **BLOCK 0 건이면** → "기획 검증 통과. `/gd-start` → `/gd-chat <scene>` 로 화면을 만드세요." 안내만 출력하고 종료(파이프라인 끝).

<!-- gd:advance next=gate -->
